...
getgenv().NamekianSlot = "Slot1"
getgenv().DesiredSlot = "Slot2"
...
task.spawn(function()
	...
	while task.wait() do 
		if game:GetService("CoreGui").RobloxPromptGui.promptOverlay:FindFirstChild("ErrorPrompt") then 
			writefile('shutdownlogs.txt','shutdown');
			game:Shutdown()
			break
		end
	end
...
local valid = isfile("shutdownlogs.txt")

if valid then 

else
	writefile('shutdownlogs.txt','')
end

if game.PlaceId == 536102540 and #game.Players:GetPlayers() ~= 1 and getgenv().StealthMode == true then 
	wait(1)
	writefile('shutdownlogs.txt','shutdown')
end
local function LevelSequence()
	...
	task.wait(3)
	if game:GetService("Workspace").FriendlyNPCs:FindFirstChild("Future Trunks") then 
	Player.Backpack.ServerTraits.ChatStart:FireServer(game:GetService("Workspace").FriendlyNPCs["Future Trunks"].Chat) --crazy stuff here
...
local function ResetSlot()
	...
	Player.Backpack.ServerTraits.ChatAdvance:FireServer({"Yes"})
	task.wait(0.35)
	if Player.Character:FindFirstChild("Race") and Player.Character:FindFirstChild("Race").Value ~= "Namekian" 	then return end
...
local function Boost()
	...
	if Player.Character.Race.Value == "Namekian" then
			...
			if Player.PlayerGui.HUD.Bottom.ChatGui.TextLabel.Text == "Alright let's do it" then
				...
				task.spawn(function()
					pcall(function()
						...
						if not Success then writefile('shutdownlogs.txt','shutdown'); game:Shutdown() end
					end)
				end)
				break
		...
		repeat 
			...
			Player.Backpack.ServerTraits.ChatAdvance:FireServer({getgenv().NamekianSlot})
			task.wait(0.2)
		until game.Players.LocalPlayer.Character:FindFirstChild('Race') and game.Players.LocalPlayer.Character:FindFirstChild('Race').Value == 'Namekian'
		-- repeat
		-- 	local isnamek = false 
		-- 	if Player.Character:FindFirstChild('Race') and  Player.Character:FindFirstChild('Race').Value == 'Namekian' then isnamek = true end
		-- 	Player.Backpack.ServerTraits.ChatStart:FireServer(workspace.FriendlyNPCs["Character Slot Changer"])
		-- 	task.wait(0.4)
		-- 	Player.Backpack.ServerTraits.ChatAdvance:FireServer({"Yes"})
		-- 	task.wait(0.4)
...
local function CheckStatus(Char)
	...
	local Race = Player.Character:WaitForChild("Race", 60)
	if not Race then
		writefile('shutdownlogs.txt','shutdown')
		game:Shutdown()
...
elseif game.PlaceId == 536102540 then
	...
	task.delay(
		...
		function()
			while true do
				task.wait(0.1)
				if TimeAbovePing > 25 and Mode == "Boost" and workspace.DistributedGameTime > 30 then writefile('shutdownlogs.txt','shutdown'); game:Shutdown() end
	...
	if not Manual then
		Player.CharacterAdded:Connect(
			function(Chara)
				local HUD = Player.PlayerGui:WaitForChild("HUD", 10)--//7
				local Race = Player.Character:WaitForChild("Race",6)
	...
	local HUD = Player.PlayerGui:WaitForChild("HUD", 25)
	local Race = Player.Character:WaitForChild("Race", 25)
	if not HUD and not Race then
		writefile('shutdownlogs.txt','shutdown');
		game:Shutdown()
	end
